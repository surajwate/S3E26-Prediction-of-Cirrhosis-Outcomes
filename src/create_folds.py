...
def create_classification_kfolds(
    file_path: str = typer.Option(
        "./input/train.csv", help="Path to the input CSV file containing the dataset."
    ),
    n_splits: int = typer.Option(
        5, help="Number of folds. Default is 5."
    ),
    target: str = typer.Option(
        ..., help="The name of the target column.", prompt=True
    ),
    random_state: int = typer.Option(
        42, help="Seed for the random number generator. Default is 42."
    ),
    save_path: str = typer.Option(
        "./input/train_folds.csv", help="Optional path to save the CSV file."
    )
):
    """
    ...
    Parameters:
    - file_path: Path to the input CSV file containing the dataset.
    - target: The name of the target column.
    - n_splits: Number of folds. Default is 5.
    - random_state: Seed for the random number generator. Default is 42.
    - save_path: Optional path to save the CSV file. Default is './input/train_folds.csv'.
...
if __name__ == "__main__":
    # create_kfolds()
    typer.run(create_classification_kfolds)
